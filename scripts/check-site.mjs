...
export const checks = [
  ...
  ['Wörterbuch: Schriftsystem passt (ar/fa arabisch, ka georgisch, ru kyrillisch ≥ 90 %), Begriff ≠ deutsches Wort (tr/en ≥ 80 %), keine Fremdschrift (CJK/Hangul), Hinweis nie bloßes Umgangswort, Markennamen lateinisch in allen 7 Sprachen, kein fremdes Rahmenwort, Lehnwort erfordert Hinweis', () => { const e = JSON.parse(readFileSync(join(src, 'data/woerterbuch.json'), 'utf8')).eintraege; const L = ['en', 'tr', 'ru', 'ar', 'fa', 'ka', 'sq']; const anteil = (l, re) => e.filter((x) => re.test(x.i18n[l].begriff + x.wofuer[l])).length / e.length; const anders = (l) => e.filter((x) => x.i18n[l].begriff.toLowerCase() !== x.de.singular.toLowerCase()).length / e.length; const fremd = /[぀-ヿ㐀-䶿一-鿿가-힯]/; const MARKEN = ['Duspol', 'Wago', 'Knipex', 'Wiha', 'Wera', 'Bosch', 'Hilti', 'Makita', 'Fluke', 'Benning']; const RAHMEN = { en: 'colloquially', tr: 'Halk arasında', ru: 'в просторечии', ar: 'تُسمى أيضاً', fa: 'در محاوره', ka: 'სასაუბროდ', sq: 'në zhargon' }; const quelle = (x) => `${x.wofuer.de} ${x.de.singular} ${x.de.umgangssprache ?? ''}`; const ziel = (x, l) => `${x.i18n[l].begriff} ${x.i18n[l].hinweis ?? ''} ${x.wofuer[l]}`; const bloss = (x, l) => { const h = x.i18n[l].hinweis; if (!h) return false; const u = (x.de.umgangssprache ?? '').trim().toLowerCase(); const hh = h.trim().toLowerCase(); return hh === u || u.split(',').map((t) => t.trim()).includes(hh); }; const fremdesRahmen = (x, l) => { const h = (x.i18n[l].hinweis ?? '').toLowerCase(); return Object.entries(RAHMEN).some(([y, w]) => y !== l && h.includes(w.toLowerCase())); }; const lehnwortOk = (x, l) => { const b = x.i18n[l].begriff.toLowerCase(); const quellen = new Set([x.de.singular.toLowerCase()]); if (x.de.umgangssprache) { quellen.add(x.de.umgangssprache.toLowerCase()); x.de.umgangssprache.split(',').forEach((t) => quellen.add(t.trim().toLowerCase())); } return !quellen.has(b) || Boolean(x.i18n[l].hinweis); }; return anteil('ar', /[؀-ۿ]/) >= 0.9 && anteil('fa', /[؀-ۿ]/) >= 0.9 && anteil('ka', /[Ⴀ-ჿ]/) >= 0.9 && anteil('ru', /[Ѐ-ӿ]/) >= 0.9 && anders('tr') >= 0.8 && anders('en') >= 0.8 && e.every((x) => L.every((l) => !fremd.test(ziel(x, l)))) && e.every((x) => L.every((l) => !bloss(x, l))) && e.every((x) => MARKEN.every((m) => !quelle(x).includes(m) || L.every((l) => ziel(x, l).includes(m)))) && e.every((x) => L.every((l) => !fremdesRahmen(x, l))) && e.every((x) => L.every((l) => lehnwortOk(x, l))); }],
];
