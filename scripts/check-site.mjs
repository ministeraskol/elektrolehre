...
export const checks = [
  ...
  ['Datenschutz sayfası: Cloudflare Pages + sessionStorage (sl-sidebar-state) + § 25 Abs. 2 Nr. 2 TDDDG, kein Wikimedia-Abschnitt', () => { const h = existsSync(page('de/rechtliches/datenschutz')) ? read('de/rechtliches/datenschutz') : ''; return h.includes('Cloudflare Pages') && !h.includes('GitHub Pages') && h.includes('sessionStorage') && [...h.matchAll(/<code\b[^>]*>([^<]*)<\/code>/g)].some((m) => m[1] === 'sl-sidebar-state') && h.includes('§ 25 Abs. 2 Nr. 2 TDDDG') && !/Wikimedia/i.test(h); }],
  ...
  ['Mobil: Drawer (starlight__sidebar) mit Tabs auf Hub-Seiten; Startseite mit eigener Leiste statt Drawer', () => read('de/entdecken').includes('id="starlight__sidebar"') && /class="ww-tabs-mobil md:sl-hidden(?: astro-[\w-]+)?"/.test(read('de/entdecken')) && !read('de').includes('id="starlight__sidebar"') && /<html[^>]*data-has-sidebar/.test(read(`de/${ARTIKEL}`))],
  ...
  ['Header: Wortmarke watt<b>was</b>, keine Social-Icons', () => { const kopf = read('de/entdecken').match(/<header[^>]*>[\s\S]*?<\/header>/)?.[0] ?? ''; return /class="ww-marke(?: astro-[\w-]+)?"[^>]*>watt<b[^>]*>was<\/b>/.test(kopf) && !kopf.includes('social-icons'); }],
  ...
  // Startseite (22.09.2026): eigene Leiste, Hero über die ganze Breite, zwölf Seiten je Stufe, drei Werkzeuge, Anmeldung.
  ['Startseite (de): ein h1 (Hero), eigene Leiste, Stufenwahl (3 im Hero + 3 in der Leiste + 3 Fotos), 9 Sprachen im Menü, Anmeldeformular', () => { const h = read('de'); return (h.match(/<h1\b/g) || []).length === 1 && /<h1 class="hero__titel" id="hero-titel"/.test(h) && /<header class="nav klein" id="ww-nav"/.test(h) && (h.match(/ data-stufe="(einstieg|azubi|profi)"/g) || []).length === 9 && (h.match(/<div class="sprache__liste"[\s\S]*?<\/div>\s*<\/div>/)?.[0].match(/hreflang="/g) || []).length === 9 && /<form class="mitglied__form" id="ww-mitglied-form"/.test(h); }],
  ['Startseite in allen 9 Locales: Hero-Titel, drei Stufen im Hero, zwölf Seiten je Stufe in den Daten, Werkzeug- und Mitglied-Abschnitt', () => ['de', 'leicht', 'tr', 'en', 'ru', 'ar', 'fa', 'ka', 'sq'].every((l) => { const h = read(l); const roh = h.match(/<script type="application\/json" id="ww-start-daten">([\s\S]*?)<\/script>/)?.[1]; if (!roh) return false; const d = JSON.parse(roh); return /<h1 class="hero__titel"/.test(h) && (h.match(/class="stufen stufen--index[\s\S]*?<\/div>/)?.[0].match(/ data-stufe="/g) || []).length === 3 && ['einstieg', 'azubi', 'profi'].every((st) => d.reihenfolge[st].length === 12 && d.reihenfolge[st].every((slug) => d.seiten[slug]?.[st]?.titel && d.hrefVon[slug] === `/${l}/${slug}/`)) && h.includes('id="werkzeug"') && h.includes('id="mitglied"'); }) && /<html[^>]*lang="de-x-leicht"/.test(read('leicht'))],
  ['Startseite: kein Starlight-Gerüst (kein Header, keine Seitenleiste, kein zweites h1), kein Video, keine Kanäle, kein Lernpfad', () => ['de', 'tr', 'ar'].every((l) => { const h = read(l); return !/<header class="header\b/.test(h) && !h.includes('id="starlight__sidebar"') && (h.match(/<h1\b/g) || []).length === 1 && !/class="video|class="[^"]*\bkanal-|ww-lernpfad|class="thema[ "]|hero-schaltkreis/.test(h); })],
  ...
  ['Leichte Sprache: /leicht/ nutzt ui.leicht (Theme-Label „Dunkel“, Tab „Fach-Wörter“), nicht ui.de', () => { const h = read('leicht/entdecken'); return !h.includes('Dunkles Design') && h.includes('Fach-Wörter'); }],
  // TP1 · Task 12
  ...
  ...(() => {
    ...
    return [
      ...
      ['SEO (e): Wortmarke verlinkt /<locale>/ mit Schluss-Slash (Startseite + Marken-Seite in 9 Locales, 404)', () => {
        // Die Startseite (22.09.) traegt eine eigene Leiste; ihre Wortmarke heisst nav__marke.
        const startMarke = (h) => attrs(h.match(/<a\b[^>]*\bclass="nav__marke\b[^"]*"[^>]*>/)?.[0] ?? '<a>').href;
        return LOCALES_9.every((l) => startMarke(html(l, '')) === `/${l}/` && wortmarke(html(l, 'elektrowerkzeuge/marken/zangen')) === `/${l}/`) && wortmarke(seite404()) === '/de/';
      }],
    ];
  })(),
  ...
  ['UI · Stufen (tr): Header-Chip (Text + Optionen), Stufen-Leiste, Startseiten-Wahl, Artikel-Pill – Azubi/Fachkraft lang="de", Başlangıç ohne', () => {
    ...
    const indexBlock = start.match(/class="stufen stufen--index[\s\S]*?<\/div>/)?.[0] ?? '';
    const wahl = (st) => indexBlock.match(new RegExp(`data-stufe="${st}"[\\s\\S]*?<span class="stufen__name"([^>]*)>([^<]*)</span>`));
  ...
  ['UI · Stufen (de/leicht): kein lang an Stufen-Labels (Seite ist selbst Deutsch) – Marken-Pills de, Header-Chip + Startseiten-Wahl leicht', () => {
    const k = read('de/elektrowerkzeuge/marken/kabelfinder'), le = read('leicht/entdecken'), leStart = read('leicht');
    return /<span class="ww-pill ww-pill-azubi/.test(k) && !/<span class="ww-pill ww-pill-(azubi|profi)[^"]*"[^>]* lang=/.test(k) && /<span class="ww-chip-text[^"]*" data-s="azubi"/.test(le) && !/<span class="ww-chip-text[^"]*" data-s="[a-z]+" lang=/.test(le) && !/<span class="stufen__name" lang=/.test(leStart);
  }],
