...
export const checks = [
  ...
  // TP1 · Task 4: Header
  ['Header: 5 Tabs (Entdecken, Lernen, Werkzeug, Glossar, Blog), kein Marken-Tab (kommt erst mit Task 7)', () => { const h = read(`de/${ARTIKEL}`); return (h.match(/class="ww-tab(?: astro-[\w-]+)?"/g) || []).length === 5 && ['/de/entdecken/', '/de/lernen/', '/de/elektrowerkzeuge/', '/de/glossar/', '/de/blog/'].every((p) => new RegExp(`class="ww-tab(?: astro-[\\w-]+)?"[^>]*href="${p}"`).test(h)) && !new RegExp(`class="ww-tab(?: astro-[\\w-]+)?"[^>]*href="/de/elektrowerkzeuge/marken/"`).test(h); }],
