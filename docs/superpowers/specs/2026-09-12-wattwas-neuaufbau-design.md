...
13. **Logische CSS-Eigenschaften:** richtungsabhängige physische Kurzschreibweisen aus dem Plan (z. B. Listen-Einzug in `Lernen.astro`) wurden in `padding-inline-*`/`margin-inline-*` übersetzt; achsensymmetrische Kurzschreibweisen bleiben. Nicht alle physischen Kurzschreibweisen im Code wurden übersetzt: `Kanaele.astro` (`.kanal`/`.gross .kanal`-Padding) behält das physische `padding`-Shorthand bis TP2 – heute unsichtbar, da kein Kanal eine `url` hat (siehe Punkt 7).
...
15. **Kennwert-Namen lokalisiert** (Nachtrag 14.09., Task 9): `marken.json.kennwerte[<deutscher Schlüssel>][locale]` für alle 42 Schlüssel aus `modelle[].specs[].k`, in 9 Locales; der deutsche Schlüssel bleibt außerhalb von `de`/`leicht` als `title`-Attribut abrufbar (Lernhilfe, Fachbegriff bleibt sichtbar). Die Werte (`specs[].v`) bleiben unverändert wie im Datenblatt – nur die Spaltenüberschrift/das `dt`-Label wird übersetzt, `specs[].k` bleibt der deutsche Schlüssel (Vergleichstabelle vereinigt weiter darüber).
16. **Stufe „Profi“ → „Fachkraft“** (Rückmeldung R2, Kadir 14.09.): Anzeige-Name der Stufe `profi` heißt „Fachkraft“ (en „Skilled worker“, leicht „Fach-Kraft“, die 6 übrigen Sprachen deutsch „Fachkraft“ wie „Azubi“), mit Hinweissatz in der Stufen-Legende, der die Lern-Stufe von der rechtlichen Elektrofachkraft nach DGUV Vorschrift 3 trennt. Der Code-Schlüssel `profi` bleibt überall unverändert (Daten, Klassen, `data-stufe`, Filter). Damit ist auch die offene Frage zu `en.stufen` aus §14.5 beantwortet: Englisch bekommt weiterhin eine eigene Übersetzung (vorher „Pro“, jetzt „Skilled worker“) statt des deutschen Begriffs.
17. **Drei weitere, bisher undokumentierte Abweichungen** (nachdokumentiert, Final-Fix-Welle 14.09.): Entdecken zeigt 4 statt 3 Werkzeug-Karten (`werkzeug` in `entdecken.json`: guide, Grundausstattung, Wörterbuch, Marken – letztere kamen mit TP2 dazu). Der Sortier-Chip im Wörterbuch heißt „Nach Kategorie“, nicht „Deutsch → UI-Sprache“ (das war nie ein UI-Text, sondern nur eine planerische Beschreibung der Zielspalte). Die Notiz auf der Marken-Kategorieseite (`marken.hinweis`, mit der Übersicht geteilt: „Keine Kaufempfehlung. Preise nur als Budgetstufe …“) nennt „keine Partner-Links“ nicht – dieser Satz steht nur im Untertitel der Marken-Übersicht (`marken.untertitel`), nicht in der (auf beiden Seiten identischen) Notiz.
18. **Generator-Check** (Final-Fix-Welle 14.09., I5): `node scripts/marken-seiten.mjs --check` rendert alle 99 Marken-Seiten im Speicher, vergleicht nur `src/content/docs/*/elektrowerkzeuge/marken/**` und schreibt nichts; bei Abweichung Exit-Code 1 + Liste der abweichenden Dateien. `npm test` ruft seither diesen Modus auf statt des schreibenden Laufs + `git status` über den ganzen Doku-Baum (der änderte auch Dateien außerhalb der Marken-Seiten). Generierte MDX tragen im Frontmatter die Kommentarzeile `# generiert von scripts/marken-seiten.mjs – nicht von Hand ändern`; `leicht` bekommt `translated: source` (deutsche Muttersprachtexte in Leichter Sprache) statt `machine`.
