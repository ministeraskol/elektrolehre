...
- [x] DNSSEC: Cloudflare'de açıldı (keytag 2371, alg 13, DS 143F9991…), DNSKEY INWX'e `dnssec.adddnskey` ile; ~09:15 DS DENIC'te, Cloudflare status **active** ✓; INWX girişi kasadan silindi
- [x] `_redirects` www-Regel entfernt (nicht unterstützt, e983591); INWX transfer kilidi açık, INWX 2FA yok (Kadir açmalı)
- [x] Kadir (dashboard, 18 Eyl ~09:10): www → Apex Redirect Rule (301 ✓) · Minimum TLS 1.2 (TLS 1.1 reddediliyor ✓) · Bot Fight Mode + AI-Bots · INWX 2FA açık
- [x] Pages-only API token (Kadir) → GitHub secrets + `CLOUDFLARE_PAGES=on`; `deploy-cloudflare.yml` run 35318144751 ✓ – her push otomatik Cloudflare'e
...
| `www.wattwas.de/de/` | 301 → `https://wattwas.de/de/` (Redirect Rule, 09:10) |
